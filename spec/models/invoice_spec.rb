...
RSpec.describe Invoice, type: :model do
  ...
  describe "instance methods" do
    ...
    it "find_merchant_discount" do
      ...
      @bulk_discount_1 = BulkDiscount.create!(name: "10% off 10 or more", discount: 10, threshold: 10, merchant_id: @merchant1.id)
      @bulk_discount_2 = BulkDiscount.create!(name: "5% off 5 items", discount: 5, threshold: 5, merchant_id: @merchant1.id)

      expect(@invoice_1.find_merchant_discount(@merchant1)).to eq(@bulk_discount_2)

      @invoice_item2.update(quantity: 10)

      expect(@invoice_1.find_merchant_discount(@merchant1)).to eq(@bulk_discount_1)
    end
    ...
    it "discounted_invoice_revenue" do
      ...
      @customer_1 = Customer.create!(first_name: 'Joey', last_name: 'Smith')
      @invoice_1 = Invoice.create!(customer_id: @customer_1.id, status: 2, created_at: "2012-03-27 14:54:09")
      @invoice_item1 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_1.id, quantity: 15, unit_price: 10, status: 2)
      @invoice_item2 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_2.id, quantity: 5, unit_price: 10, status: 1)
      @bulk_discount_1 = BulkDiscount.create!(name: "10% off 10 or more", discount: 0.10, threshold: 10, merchant_id: @merchant1.id)

      expect(@invoice_1.discounted_invoice_revenue(@merchant1, @bulk_discount_1)).to eq(@invoice_1.merchant_invoice_revenue(@merchant1) * 0.9)

      @bulk_discount_1.update(discount: 0.20)

      expect(@invoice_1.discounted_invoice_revenue(@merchant1, @bulk_discount_1)).to eq(@invoice_1.merchant_invoice_revenue(@merchant1) * 0.8)
    end
  end
end
